...
async function handleApi(req, res, url) {
  if (req.method === 'GET' && url.pathname === '/api/health') return sendJson(res, 200, { ok:true, version:'23.0.0', dataDir:DATA_DIR });
  if (req.method === 'GET' && url.pathname === '/api/config') {
    const store = readStore(); const s = store.settings; s.tutorials=s.tutorials||{orderUrl:'',gamepassUrl:''}; s.contactEmail=s.contactEmail||'reckshopemergencycontact@gmail.com'; s.facebookUrl=s.facebookUrl||'https://www.facebook.com/profile.php?id=61592793360824';
    return sendJson(res, 200, { ...s, shopName:s.shopName || process.env.SHOP_NAME || 'RSR SHOP', contactEmail:s.contactEmail || process.env.CONTACT_EMAIL || '', contactPhone:s.contactPhone || process.env.CONTACT_PHONE || '', businessLocation:s.businessLocation || process.env.BUSINESS_LOCATION || 'Philippines', facebookUrl:s.facebookUrl || process.env.FACEBOOK_URL || '' });
  }
  ...
  if (req.method === 'POST' && url.pathname === '/api/roblox/gamepass') {
    const body=await parseBody(req); const raw=String(body.input||'').trim();
    const match=raw.match(/game-pass\/(\d+)/i)||raw.match(/gamepass\/(\d+)/i)||raw.match(/^(\d+)$/); const id=match?.[1]||'';
    if(!id)return sendJson(res,400,{error:'Paste a valid Roblox gamepass link or enter only the gamepass ID.'});
    try {
      // Roblox gamepasses use the public game-pass product-info endpoint, not the normal asset-details endpoint.
      const item=await fetchJson(`https://apis.roblox.com/game-passes/v1/game-passes/${id}/product-info`);
      const price=Number(item.PriceInRobux??item.priceInRobux??item.price??0);
      const creator=item.Creator?.Name||item.creator?.name||item.CreatorName||'Unknown';
      const name=item.Name||item.name||`Gamepass ${id}`;
      const isForSale=Boolean(item.IsForSale??item.isForSale??price>0);
      let thumbnail='';
      try{const thumbs=await fetchJson(`https://thumbnails.roblox.com/v1/game-passes?gamePassIds=${id}&size=150x150&format=Png&isCircular=false`);thumbnail=thumbs.data?.[0]?.imageUrl||'';}catch{}
      return sendJson(res,200,{gamepass:{id:Number(id),name,price,creator,productId:item.ProductId||item.productId||null,isForSale,thumbnail,url:`https://www.roblox.com/game-pass/${id}`}});
    }
    catch(error){ return sendJson(res,502,{error:`Gamepass verification unavailable: ${error.message}. Confirm the gamepass ID is correct and that the pass is publicly available.`}); }
  }
  ...
  if (req.method==='POST' && url.pathname==='/api/orders') {
    ...
    if(method==='gifting'){if(!body.game?.placeId)return sendJson(res,400,{error:'Verify the Roblox game first.'});const g=body.giftingDetails||{};if(!g.recipientUser?.id||!g.recipientUser?.name)return sendJson(res,400,{error:'Verify the Roblox username receiving the gift.'});if(String(g.recipientUsername||'').trim().toLowerCase()!==String(g.recipientUser.name||'').trim().toLowerCase())return sendJson(res,400,{error:'The recipient username changed after verification. Verify it again.'});const items=Array.isArray(g.items)?g.items.filter(x=>String(x?.name||'').trim().length>=2):[];if(!items.length)return sendJson(res,400,{error:'Add at least one valid item name to gift.'});if(items.some(x=>Number(x.quantity||0)<1))return sendJson(res,400,{error:'Every gift item must have a valid quantity.'});}
    const rate=Number(settings.rates[method]||0); const totalPhp=Math.round(amount*rate*100)/100;
    const dateCode=new Date().toISOString().slice(0,10).replaceAll('-','');const orderNo=`RSR-${dateCode}-${String(store.orders.length+1).padStart(5,'0')}`;const transactionId=`RSR-TXN-${dateCode}-${crypto.randomBytes(3).toString('hex').toUpperCase()}`;const order={id:newId('ord'),orderNo,transactionId,userId:dbUser.id,customerName:dbUser.name,customerEmail:dbUser.email,method,robloxUser:body.robloxUser,robuxAmount:amount,requiredGamepass,gamepasses:body.gamepasses||[],game:body.game||null,giftingDetails:body.giftingDetails?{recipientUsername:String(body.giftingDetails.recipientUsername||'').trim().slice(0,50),recipientUser:{id:Number(body.giftingDetails.recipientUser?.id||0),name:String(body.giftingDetails.recipientUser?.name||'').slice(0,50),displayName:String(body.giftingDetails.recipientUser?.displayName||'').slice(0,80),avatar:String(body.giftingDetails.recipientUser?.avatar||'').slice(0,500),profileUrl:String(body.giftingDetails.recipientUser?.profileUrl||'').slice(0,500)},items:(Array.isArray(body.giftingDetails.items)?body.giftingDetails.items:[]).slice(0,20).map(x=>({name:String(x?.name||'').trim().slice(0,120),quantity:Math.max(1,Math.min(999,Math.floor(Number(x?.quantity||1)))),itemLink:String(x?.itemLink||'').trim().slice(0,500)})).filter(x=>x.name.length>=2),instructions:String(body.giftingDetails.instructions||'').trim().slice(0,500)}:null,totalPhp,paymentMethod:String(body.paymentMethod),receiptData:String(body.receiptData),receiptVerified:false,status:'Pending Review',adminNote:'',deliveryProof:'',timeline:[{status:'Pending Review',at:new Date().toISOString(),by:dbUser.name}],createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};
  ...
  if(req.method==='GET'&&url.pathname==='/api/admin/settings'){
    if(dbUser.role!=='admin')return sendJson(res,403,{error:'Admin access required.'});
    store.settings.tutorials=store.settings.tutorials||{orderUrl:'',gamepassUrl:''};store.settings.contactEmail=store.settings.contactEmail||'reckshopemergencycontact@gmail.com';store.settings.facebookUrl=store.settings.facebookUrl||'https://www.facebook.com/profile.php?id=61592793360824'; return sendJson(res,200,{settings:store.settings});
  }
  if(req.method==='PATCH'&&url.pathname==='/api/admin/settings'){
    ...
    store.settings.minRobux=min;store.settings.maxRobux=max;store.settings.tutorials=store.settings.tutorials||{orderUrl:'',gamepassUrl:''};if(body.tutorials){for(const key of ['orderUrl','gamepassUrl']){const value=String(body.tutorials[key]||'').trim().slice(0,1000);if(value&&!/^https?:\/\//i.test(value))return sendJson(res,400,{error:'Tutorial links must start with http:// or https://'});store.settings.tutorials[key]=value;}}const fb=String(body.facebookUrl||store.settings.facebookUrl||'').trim().slice(0,1000);const email=String(body.contactEmail||store.settings.contactEmail||'').trim().toLowerCase().slice(0,200);const announcement=String(body.announcement||store.settings.announcement||'').trim().slice(0,240);if(fb&&!/^https?:\/\//i.test(fb))return sendJson(res,400,{error:'Facebook link must start with http:// or https://'});if(email&&!/^\S+@\S+\.\S+$/.test(email))return sendJson(res,400,{error:'Enter a valid support email.'});store.settings.facebookUrl=fb;store.settings.contactEmail=email;store.settings.announcement=announcement;writeStore(store);return sendJson(res,200,{settings:store.settings});
  }
