...
async function handleApi(req, res, url) {
  if (req.method === 'GET' && url.pathname === '/api/health') return sendJson(res, 200, { ok:true, version:'22.5.0', dataDir:DATA_DIR });
  if (req.method === 'GET' && url.pathname === '/api/config') {
    const store = readStore(); const s = store.settings; s.tutorials=s.tutorials||{orderUrl:'',gamepassUrl:''};
  ...
  if(req.method==='GET'&&url.pathname==='/api/admin/settings'){
    if(dbUser.role!=='admin')return sendJson(res,403,{error:'Admin access required.'});
    store.settings.tutorials=store.settings.tutorials||{orderUrl:'',gamepassUrl:''}; return sendJson(res,200,{settings:store.settings});
  }
